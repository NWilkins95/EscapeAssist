...
from workflows.V2workflow import run_workflow, WorkflowInput
...
def load_workflow():
    """
    Cache the V2 workflow function for Streamlit.
    """
    return run_workflow
...
st.header("Welcome to EscapeAssist V2!")
st.markdown("My knowledge is based on the cleaned, preprocessed 2022 Ford Escape Owner's Manual.")
...
if prompt:
    ...
    if "conversation_history" in result:
        st.session_state.conversation_history_v2 = result["conversation_history"]

    # Add to UI history
    st.session_state.messages_v2.append({"role": "assistant", "content": reply})