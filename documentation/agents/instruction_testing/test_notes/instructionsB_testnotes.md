# EscapeAssist – Instruction Variation Testing

---

## Instruction Version Being Tested: 
- Date: 

---

## 1. Questions Tested  
(Use 5–7 from the Week 4 subset)

1.  
2.  
3.  
4.  
5.  
6.  
7.  

---

## 2. Observations Per Question

### Question 1:
**User Question:**  
**Grounding Behavior:**  
**Safety Behavior:**  
**Tone & Style:**  
**Failures Noticed:**  

---

### Question 2:
**User Question:**  
**Retrieved Chunks Observed:**  
**Grounding Behavior:**  
**Safety Behavior:**  
**Tone & Style:**  
**Failures Noticed:**  

---

### Question 3:
**User Question:**  
**Retrieved Chunks Observed:**  
**Grounding Behavior:**  
**Safety Behavior:**  
**Tone & Style:**  
**Failures Noticed:**  

---

### Question 4:
**User Question:**  
**Retrieved Chunks Observed:**  
**Grounding Behavior:**  
**Safety Behavior:**  
**Tone & Style:**  
**Failures Noticed:**  

---

### Question 5:
**User Question:**  
**Retrieved Chunks Observed:**  
**Grounding Behavior:**  
**Safety Behavior:**  
**Tone & Style:**  
**Failures Noticed:**  

---

## (Optional) Question 6:
**User Question:**  
**Retrieved Chunks Observed:**  
**Grounding Behavior:**  
**Safety Behavior:**  
**Tone & Style:**  
**Failures Noticed:**  

---

## (Optional) Question 7:
**User Question:**  
**Retrieved Chunks Observed:**  
**Grounding Behavior:**  
**Safety Behavior:**  
**Tone & Style:**  
**Failures Noticed:**  

---

# 3. Summary of Grounding Behavior for This Instruction Version
(How well did this version stay within retrieved content?  
Did it hallucinate? Contradict the manual?)

-  
-  
-  

---

# 4. Summary of Safety Behavior
(Did it refuse unsafe requests correctly?  
Did it avoid mechanical diagnosis?  
Did it avoid repair instructions?)

-  
-  
-  

---

# 5. Summary of Tone & Style
(Did the tone match the intended style of this instruction version?  
Was the answer readable and structured?)

-  
-  
-  

---

# 6. Failure Cases Identified
(List specific failures you observed. These will become your Week 5–6 categories.)

- Hallucinations:  
- Incorrect refusals:  
- Overly strict refusals:  
- Tone mismatches:  
- Unsafe suggestions:  
- Other failures:  

---

# 7. Overall Impression of This Instruction Version
(Write your high‑level takeaways.)

- Strengths:  
- Weaknesses:  
- Situations where this version performs best:  
- Situations where it struggles:  

---

# 8. Should This Version Be a Candidate for the Final Instructions?
(Yes / No / Maybe — and why)

-  

---